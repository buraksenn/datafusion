...
/// [`Self::push_batch`] and [`Self::flush_buffered_batch`] is enforced against
/// the pool limit and surfaces as a [`DataFusionError::ResourcesExhausted`]
...
impl LimitedBatchCoalescer {
    ...
    /// Create a coalescer that emits exact target-sized batches while charging
    /// retained memory to `reservation`.
    ...
    /// Return true if there is a completed batch ready to be fetched.
    ...
    /// Transfer the next completed batch's accounting to `destination` before
    /// releasing this coalescer's charge.
    ///
    /// The returned byte count is the charge held by `destination`; callers
    /// must shrink that amount when they release the batch. The transfer
    /// itself is infallible (the memory is already charged to this coalescer;
    /// it is only double-counted for the instant between growing
    /// `destination` and reconciling this reservation).
    pub fn next_completed_batch_with_reservation(
        &mut self,
        destination: &MemoryReservation,
        ...
        destination.grow(charged_bytes);
        self.reconcile_reservation();
        Some((batch, charged_bytes))
    }
...
mod tests {
    ...
    #[test]
    fn dequeue_transfers_reservation_until_destination_releases_batch() {
        let batch = uint32_batch(0..2);
        let pool: Arc<dyn MemoryPool> = Arc::new(UnboundedMemoryPool::default());
        let source = MemoryConsumer::new("LimitedBatchCoalescerTest").register(&pool);
        let destination =
            MemoryConsumer::new("LimitedBatchCoalescerDestination").register(&pool);
        let mut coalescer =
            LimitedBatchCoalescer::new_with_reservation(batch.schema(), 4, None, source);
        ...
        let (actual, charged_bytes) = coalescer
            .next_completed_batch_with_reservation(&destination)
            .unwrap();
        assert_eq!(actual, batch);
        assert!(charged_bytes > 0, "the dequeued batch must remain charged");
        assert_eq!(charged_bytes, completed_size - baseline);
        assert_eq!(destination.size(), charged_bytes);
        assert_eq!(pool.reserved(), completed_size);

        destination.shrink(charged_bytes);
    ...
    #[test]
    fn exact_coalescer_splits_oversized_batch_and_releases_reservation() {
        let target_batch_size = 2;
        ...
        // Prime Arrow's reusable output queue so the baseline includes retained capacity.
        coalescer.push_batch(input.clone()).unwrap();
        ...
        let output_sizes = output_batches
            .iter()
            .map(RecordBatch::num_rows)
            .collect::<Vec<_>>();
        assert_eq!(output_sizes, vec![2, 2, 1]);
        assert!(
            output_batches[..output_batches.len() - 1]
                .iter()
                .all(|batch| batch.num_rows() <= target_batch_size),
            "non-final output batches must not exceed the exact target"
        );
        let combined = concat_batches(&input.schema(), &output_batches).unwrap();
        assert_eq!(
            batch_to_pretty_strings(&combined),
            batch_to_pretty_strings(&input),
            "splitting must preserve input row order and content"
        );
        assert_eq!(pool.reserved(), baseline);
    ...
    fn bypass_batches_are_accounted_and_released() {
        ...
        assert!(
            ...
            "an oversized batch must pass through as a completed batch"
        );
