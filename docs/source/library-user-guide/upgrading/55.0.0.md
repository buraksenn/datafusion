...
### `LimitedBatchCoalescer` requires a `MemoryReservation` and enforces the memory pool limit

`LimitedBatchCoalescer` (in `datafusion::physical_plan::coalesce`) now accounts
for every buffered and completed batch it retains in a
[`MemoryReservation`](https://docs.rs/datafusion/latest/datafusion/execution/memory_pool/struct.MemoryReservation.html)
(see [#23385](https://github.com/apache/datafusion/issues/23385)):

- `LimitedBatchCoalescer::new` is deprecated in favor of
  `LimitedBatchCoalescer::new_with_reservation`, which takes the
  `MemoryReservation` that retained batches are charged to. `new` keeps
  working, but charges retained batches to a private unbounded pool that the
  query's memory limits cannot see. A new `new_exact_with_reservation`
  constructor emits exactly target-sized output batches (disabling the
  large-batch pass-through optimization).
- `push_batch` and the new `flush_buffered_batch` method return
  `DataFusionError::ResourcesExhausted` when buffering the data would exceed
  the memory pool limit. Such errors are terminal: stop pushing and drop the
  coalescer to release its reservation. Operators that handle memory pressure
  by spilling can use `with_unenforced_accounting`, which records the bounded,
  unspillable buffer infallibly so spilling queries keep succeeding under tight
  limits.
- `finish` is now idempotent; `flush_buffered_batch` completes the current
  buffered batch without closing the coalescer.
- The new `next_completed_batch_with_reservation` method transfers a completed
  batch's charge to a caller-provided reservation for hand-offs across
  buffering boundaries.

```rust,ignore
// Before
let coalescer = LimitedBatchCoalescer::new(schema, batch_size, fetch);

// After
let reservation =
    MemoryConsumer::new("MyOperator").register(context.memory_pool());
let coalescer = LimitedBatchCoalescer::new_with_reservation(
    schema, batch_size, fetch, reservation,
);
```

In addition, operators that buffer batches through a coalescer now register
memory consumers named after that buffer (for example `FilterExecOutput[0]`,
`CoalesceBatchesOutput[0]`, `HashJoinOutput[1]`, and `AsyncFuncInput[0]`), which
may appear in "Resources exhausted" error messages listing the top memory
consumers.
