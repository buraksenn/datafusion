...
### Protobuf plan serialization rejects out-of-range integers

Protobuf plan (de)serialization converted wire integers to `usize` with
unchecked `as` casts. On 32-bit targets any value above the pointer width
silently truncated (a fetch of `1 << 32` decoded as `0`), and encode paths
narrowed `usize` values into `uint32`/`int64` fields the same way on every
target. Both directions now return an `Execution` error naming the plan node
and field instead of producing a silently wrong plan.

As part of this, several generated fields that carry row counts or byte sizes
were widened from `uint32` to `uint64` (`LocalLimitExecNode::fetch`,
`GlobalLimitExecNode::skip`, `FilterExecNode::batch_size` and `fetch`,
`CoalesceBatchesExecNode::target_batch_size` and `fetch`,
`CoalescePartitionsExecNode::fetch`, `MemoryScanExecNode::fetch`,
`GenerateSeriesNode::target_batch_size`, and `ScanLimit::limit`). The change is
wire-compatible in both directions: `uint32` and `uint64` share the same varint
encoding. `LimitNode::fetch` is now encoded as `-1` when unset — the negative
sentinel its comment always documented — instead of `i64::MAX`, and both
spellings decode as "no limit".

**Who is affected:**

- Users constructing the widened protobuf structs with exhaustive struct
  literals: the field types changed from `u32` to `u64`.
- Users calling `CsvOptions::from_proto`, `JsonOptions::from_proto`, or
  `ParquetCdcOptions::from_proto` in `datafusion-proto`: these conversions are
  now fallible and moved from `FromProto` to `TryFromProto`.
- Users of the `datafusion-proto-common` conversions
  `From<protobuf::Constraint>`, `From<protobuf::Constraints>`, and
  `From<protobuf::ParquetCdcOptions>`: these are now `TryFrom` so decoded
  column indices and chunk sizes can report overflow (and a missing
  `constraint_mode` is an error instead of a panic).
- Users serializing plans through the protobuf JSON mapping: the widened
  fields serialize as JSON strings rather than numbers, per the standard
  protobuf JSON encoding of 64-bit integers.
...
let constraints: Constraints = proto_constraints.try_into()?;
```
